    ...
    export default function Create() {
        ...
        // Validasi frontend: telepon harus mulai 0 atau 62 dan hanya angka
        const isPhoneValid = (phone) => /^((0)|(62))\d*$/.test(phone) || phone === '';

        // Validasi frontend: email valid
        const isEmailValid = (email) =>
            /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || email === '';

        return (
            <div className="p-6 max-w-4xl mx-auto">
                <h1 className="text-3xl font-bold mb-6">Buat Invoice Baru</h1>
                ...
                <form onSubmit={submit} className="space-y-6 bg-white p-6 rounded-xl shadow-md">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-2">
                            ...
                            {errors.invoice_date && (
                                <Alert variant="destructive">
                                    <AlertDescription>{errors.invoice_date}</AlertDescription>
                                </Alert>
                            )}
                        </div>
                    ...
                    <div className="space-y-2">
                        ...
                        {errors.referensi && (
                            <Alert variant="destructive">
                                <AlertDescription>{errors.referensi}</AlertDescription>
                            </Alert>
                        )}
                    </div>
                        ...
                        {data.items.map((it, idx) => (
                            <div key={idx} className="flex gap-2 items-center mb-2">
                                ...
                                {errors[`items.${idx}.name`] && (
                                    <Alert variant="destructive">
                                        <AlertDescription>{errors[`items.${idx}.name`]}</AlertDescription>
                                    </Alert>
                                )}
                                ...
                                {errors[`items.${idx}.qty`] && (
                                    <Alert variant="destructive">
                                        <AlertDescription>{errors[`items.${idx}.qty`]}</AlertDescription>
                                    </Alert>
                                )}
                                ...
                                {errors[`items.${idx}.price`] && (
                                    <Alert variant="destructive">
                                        <AlertDescription>{errors[`items.${idx}.price`]}</AlertDescription>
                                    </Alert>
                                )}
